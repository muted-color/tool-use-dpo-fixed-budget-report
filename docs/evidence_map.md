...
This document maps the paper-facing evidence names to released artifacts and
internal trace IDs. The trace IDs are kept to make results auditable, but the
public interpretation should use the report evidence names.
...
| Pair-quality diagnostics | Rejected-output source quality is audited as diagnostic evidence, not as human ground truth or downstream matched-DPO proof. | `artifacts/stage4/stage3b/`, `docs/audit_rubric.md`, `docs/claims_and_limitations.md` | Stage3b diagnostic audit |
...
| Direct-answer coverage audit | Public When2Call labeled configs used here do not expose direct-answer gold rows, so direct-answer behavior remains a limitation. | `artifacts/stage7/direct_answer/`, `docs/direct_answer_coverage.md` | Stage7 audit |
